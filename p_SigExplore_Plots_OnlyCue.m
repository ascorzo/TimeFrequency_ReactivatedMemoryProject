...
m_Gabor.CueNight = load('m_Gabor_CueNight_AllChans_CleanData.mat');
...
subjectsCue = subjectsCue(2:end);
%subjectsPlac = fieldnames(m_Gabor.PlaceboNight.m_GaborPlacebo_total);
s_TimeStep = 0.5;
baselineTime = 5:s_TimeStep:10;
v_time = -15:0.5:15-0.5;
...
% hold on
% yyaxis right
% plot(v_time2,[stats_m_Delta_MeanAll_cue.pval],'-g','DisplayName','p-value')
% ylim([0 1])
...
% hold on
% yyaxis right
% plot(v_time2,[stats_m_Delta_MeanAll_placebo.pval],'-g','DisplayName','p-value')
% ylim([0 1])
...
vlims = [min(min(Mean_All_Cue(:),Mean_All_Sham_CN(:))),...
    max(max(Mean_All_Cue(:),Mean_All_Sham_CN(:)))]*0.3;
...
baseline = mean(Data(:,:,(baselineTime/s_TimeStep),:,:),3);
