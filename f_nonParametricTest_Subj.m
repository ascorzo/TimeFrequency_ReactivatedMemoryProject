function stats = f_nonParametricTest_Subj(ROI,xMeassure,yMeassure,condition1,condition2,data1,data2,Xt,color,start_point,end_sample)
...
[stats, ~, pvals, ~] = statcond( data,'paired', paired,...
    'method',method,'naccu',perms);%,'alpha',0.0005,'cluster',cluster);

[~, pmask] = fdr(pvals,0.0011,'nonParametric');
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
p1 = shadedErrorBar(Xt,mean(data1,1),std(data1,[],1),'lineprops', color);hold on;
p2 = shadedErrorBar(Xt,mean(data2,1),std(data2,[],1),'lineprops', '-k');hold on;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
